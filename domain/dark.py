...
核心功能：对一张输入图，依据 HSV 的 V 的分布筛选暗部；
在每个色块图内，计算连通域的 V 低分位阈值，输出暗部白掩码。

封装目标：
- 处理单个色块图，返回暗部白掩码 (uint8, 0/255)
- 批量处理 static/color_blocks/ 下的色块图，将结果保存到指定目录（如 static/dark_details/），文件名一一对应
...
def dark_mask_for_block(
    block_bgr: np.ndarray,
    *,
    v_percentile: float = 0.30,
    morph_kernel_size: int = 1,
    min_region_area: int = 10,
) -> Tuple[np.ndarray, List[Tuple[int, float]]]:
    """
    对单个“色块图”计算暗部白掩码（暗部=255，背景=0）。
    ...
    参数:
        - v_percentile: V 通道的分位数阈值（更低更暗，默认 0.30 表示取该区域内 30% 分位及以下为暗部）
        - morph_kernel_size: 暗部掩码的开运算核大小（去噪）
        - min_region_area: 连通域最小面积，过滤过小的暗部区域

    返回:
        - white_mask: HxW uint8 掩码（暗部=255，背景=0）
        - thresholds: list[(label_id, thr_v)] 每个连通域的 V 阈值
    """
...
def process_dark_color_blocks_directory(
    input_dir: str,
    output_dir: str,
    *,
    v_percentile: float = 0.30,
    ...
    """
    扫描 input_dir 下的色块图（如 .png/.jpg），逐一生成暗部白掩码，
...
__all__ = [
    "dark_mask_for_block",
    "process_dark_color_blocks_directory",
]